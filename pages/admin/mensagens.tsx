...
export default function Mensagens() {
  ...
  return <AdminLayout title="Mensagens">
    ...
    <div className="card overflow-x-auto"><table className="w-full min-w-[780px] text-left text-sm"><thead className="border-b bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="px-6 py-4">Remetente</th><th className="px-6 py-4">Assunto</th><th className="px-6 py-4">Data</th><th className="px-6 py-4">Status</th><th className="px-6 py-4 text-right">Ações</th></tr></thead><tbody>
      {loading && Array.from({ length: 6 }).map((_, index) => <tr className="border-b last:border-0" key={index}><td className="px-6 py-4"><Skeleton className="h-4 w-32 bg-slate-200" /><Skeleton className="mt-2 h-3 w-40 bg-slate-200" /></td><td className="px-6 py-4"><Skeleton className="h-4 w-52 bg-slate-200" /></td><td className="px-6 py-4"><Skeleton className="h-4 w-24 bg-slate-200" /></td><td className="px-6 py-4"><Skeleton className="h-6 w-16 rounded-full bg-slate-200" /></td><td className="px-6 py-4"><div className="flex justify-end gap-2"><Skeleton className="h-9 w-9 rounded-lg bg-slate-200" /><Skeleton className="h-9 w-9 rounded-lg bg-slate-200" /></div></td></tr>)}
      {!loading && items.length === 0 && <tr><td colSpan={5} className="px-6 py-10 text-center text-slate-500">Nenhuma mensagem recebida.</td></tr>}
