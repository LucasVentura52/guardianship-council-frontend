...
export default function Sugestoes() {
  ...
  return <AdminLayout title="Sugestões da Comunidade">
    ...
    <div className="card overflow-x-auto"><table className="w-full min-w-[850px] text-left text-sm"><thead className="border-b bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="px-5 py-4">Nome</th><th className="px-5 py-4">Assunto</th><th className="px-5 py-4">Mensagem</th><th className="px-5 py-4">Status</th><th className="px-5 py-4">Ações</th></tr></thead><tbody>
      ...
      {items.map(item => <tr className="border-b last:border-0" key={item.id}><td className="px-5 py-5"><strong className="block">{item.nome}</strong><small className="text-slate-400">{item.email}</small></td><td className="px-5 py-5">{item.assunto}</td><td className="max-w-sm px-5 py-5 text-slate-500"><p className="line-clamp-2">{item.mensagem}</p></td><td className="px-5 py-5"><span className={`status ${item.status === 'aprovada' ? 'bg-emerald-50 text-emerald-700' : item.status === 'reprovada' ? 'bg-rose-50 text-rose-700' : 'bg-amber-50 text-amber-700'}`}>{item.status}</span></td><td className="px-5 py-5"><button title="Analisar sugestão" onClick={() => review(item)} className="grid h-9 w-9 place-items-center rounded border text-blue-600"><Icon name="eye" size={16} /></button></td></tr>)}
    </tbody></table></div>
    {selected && <div className="fixed inset-0 z-[70] grid place-items-center overflow-y-auto bg-slate-950/55 p-5" role="dialog" aria-modal="true" aria-labelledby="titulo-sugestao">
      <section className="my-8 w-full max-w-2xl rounded-2xl bg-white p-7 shadow-2xl">
        ...
        <div className="mt-7 flex flex-wrap justify-end gap-3"><button className="btn-secondary" type="button" onClick={() => setSelected(null)}>Cancelar</button><button className="inline-flex items-center gap-2 rounded-lg bg-rose-600 px-5 py-3 text-sm font-bold text-white disabled:opacity-60" type="button" disabled={saving} onClick={() => moderate('reprovar')}>Reprovar</button><button className="btn-primary disabled:opacity-60" type="button" disabled={saving} onClick={() => moderate('aprovar')}><Icon name="check" />Aprovar</button></div>
      </section>
    </div>}
  </AdminLayout>;
}
