...
export default function DetalheCampanha({ campanha }: { campanha: Campanha }) {
  return <PublicLayout title={campanha.titulo}>
    ...
    <article className="container-app grid gap-10 py-14 lg:grid-cols-[1fr_320px]"><div><h2 className="section-title">Sobre a campanha</h2><div className="mt-5 whitespace-pre-line leading-8 text-slate-600">{campanha.conteudo}</div></div><aside className="card h-fit p-6"><h3 className="font-extrabold">Precisa de ajuda?</h3><p className="mt-3 text-sm leading-6 text-slate-600">O Disque 100 recebe denúncias gratuitamente, 24 horas por dia.</p><Link href="/como-acionar" className="btn-primary mt-5 w-full justify-center"><Icon name="phone" /> Como acionar</Link></aside></article>
  </PublicLayout>;
}
